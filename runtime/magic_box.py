...
class RuntimeRelay:
    """Minimal host-I/O adapter for the container runtime.

    Governance never depends on a particular host or I/O technology. A
    deployment can replace this adapter with GPIO, CAN, serial, USB, etc.
    """
...
class MagicBox:
    """Long-running runtime facade around the invariant-driven PoC core."""

    def __init__(self) -> None:
        self.gie = GIE()
        self.relay = RuntimeRelay()
        self.path = GovernedPhysicalPath(self.gie, env_id=0, actuator=self.relay)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/health":
            ...
            return
        if self.path == "/state":
            self._json(200, {"status": "ok", "authority_epoch": BOX.gie.current_epoch(0), "relay_state": BOX.relay.state})
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if self.path == "/execute":
                ...
                return
            self._json(404, {"error": "not_found"})
        except (KeyError, ValueError, TypeError, base64.binascii.Error, json.JSONDecodeError) as exc:
