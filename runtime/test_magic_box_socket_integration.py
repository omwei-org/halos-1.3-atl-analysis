...
class RecordingHost:
    ...
    def start(self) -> None:
        self.thread.start(); self.ready.wait(timeout=1)
    def join(self) -> None:
        self.thread.join(timeout=1)
    def serve_once(self) -> None:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as server:
            server.bind(self.path); server.listen(8); self.ready.set()
...
def test_magic_box_allow_crosses_socket_as_committed_envelope(tmp_path) -> None:
    path = str(tmp_path / "magic-box-io.sock"); host = RecordingHost(path); host.start()
    box = MagicBox(actuator=UnixSocketActuator(path)); epoch = box.authorize(0)
    result = box.execute(b"RELAY:ON", epoch, SafetyDecision.ALLOW, command_id="cmd-test-001"); host.join()
...
def test_magic_box_block_does_not_cross_socket(tmp_path) -> None:
    ...
    box = MagicBox(actuator=UnixSocketActuator(path)); epoch = box.authorize(0); box.revoke(0)
