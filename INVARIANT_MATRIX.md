...
| I-02 | Authority originates only from GIE-owned state. | GIE | GIE authority context/state. | GREEN | Keep architectural assertion/documentation. |
| I-03 | Explicit `AuthorityContext` cannot mutate GIE state. | GIE | State before/after explicit-context check. | GREEN | Preserve intentional test/offline escape hatch. |
| I-04 | Every execution is bound to an `env_id`. | Path / GIE / Commit Gate | `env_id` carried through result and boundary. | GREEN | Keep environment-bound tests. |
...
| I-08 | Authority is bound to the exact execution digest. | Commit Gate | Authority digest == execution digest. | GREEN | Keep direct authority-digest mismatch test. |
| I-09 | Safety is independently bound to the same execution digest. | Commit Gate | Safety digest == execution digest. | GREEN | Keep independent safety-digest mismatch test. |
...
| I-14 | Reauthorization creates a new valid epoch. | GIE | New authority epoch accepted. | GREEN | Keep positive reauthorization case. |
...
| I-17 | The SDM packet is opaque to Equinibrium. | SDM adapter | Byte-preserving adapter; no field interpretation. | GREEN | Keep opaque-payload test and code contract. |
| I-18 | SDM → ATL preserves the exact original bytes. | SDM / ATL | Packet equality. | GREEN | Keep exact-byte test. |
...
| I-29 | Authorization state and architectural effect are atomic/consistent at the hardware execution boundary. | Production GIE / Commit Gate | Hardware timing/retirement semantics. | ARCHITECTURAL | Production hardware verification requirement. |
| I-30 | Reset invalidates stale execution evidence. | Scheduler / integration layer | Reset state transition invalidates buffered action epoch. | GREEN | Keep reset negative test. |
| I-31 | Reset does not create authority. | Scheduler / GIE | Authority state remains revoked/no-authority after reset. | GREEN | Keep negative reset test. |
| I-32 | BLOCK does not require stopping upstream inference. | Scheduler | Upstream fetch continues while cached execution is blocked. | GREEN | Keep integration test. |
| I-33 | BLOCK produces a safe hold action where the integration contract requires one. | Scheduler / actuator adapter | Hold action from last authorized/current state. | GREEN | Keep explicit hold-action tests. |
| I-34 | Enforcement occurs per action, not only when a cached chunk is fetched. | Scheduler | Authority epoch checked for each selected cached action. | GREEN | Keep mid-chunk revoke/epoch tests. |
| I-35 | Scheduler/execution metadata is evidence only; scheduler cannot inject authority. | Scheduler → GIE | GIE-owned authority state remains authoritative. | GREEN | Keep arena-security negative test. |
| I-36 | Equinibrium does not parse, rewrite, or regenerate ATL packet fields. | SDM / ATL | Opaque byte path plus exact-byte preservation. | GREEN | Keep structural/code-level contract. |

## Proof status

The current PoC has reached a coherent **invariant-complete software proof boundary** for the modeled execution path. I-01 through I-26 and I-30 through I-36 are represented by executable behavior and/or dedicated tests. I-27 through I-29 remain intentionally architectural because they require a trusted hardware execution boundary and cannot be honestly claimed as proven by this Python PoC.

The proof claim is therefore deliberately bounded:

> **Within the modeled software execution path, an execution object can cross the SDM/ATL boundary only when GIE authority and Halos safety both ALLOW for the same environment, current authority epoch, and exact execution identity.**

This is not a claim that the PoC proves physical-world enforcement or hardware atomicity. Those are the next engineering boundary.
...
The test strategy is now to protect the established invariant set rather than expand the PoC horizontally. New tests should only be added when they demonstrate a distinct failure mode or close a production-boundary assumption.

1. Run the complete suite in a real Python/IsaacLab environment and record the result.
2. Preserve I-01 through I-26 and I-30 through I-36 as regression invariants.
3. Keep I-27 through I-29 explicitly architectural until hardware enforcement exists.
4. Do not redirect the PoC toward a specific robot integration; use robot scenarios only as reference demonstrations.
5. The next engineering phase should address the transition from this software proof boundary to a hardware-enforced Commit Gate.
