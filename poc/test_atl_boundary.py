...
def test_wrong_packet_identity_cannot_cross_boundary():
    ...
    boundary = ATLCommitBoundary(CommitGate())
    different = ATLExecutionObject(env_id=0, packet=bytes(reversed(range(64))), governance_epoch=481)
...
def test_cross_environment_authority_cannot_cross_atl_boundary():
    _, execution, authority, _ = _inputs()
    boundary = ATLCommitBoundary(CommitGate())
    other_environment = ATLExecutionObject(env_id=1, packet=execution.packet, governance_epoch=execution.governance_epoch)
