...
def test_h1_generated_does_not_imply_authority():
    gie = GIE()
    result = gie.check(env_id=0, action=torch.tensor([1.0, 2.0]), action_epoch=1)
    assert result.decision is Decision.BLOCK
    assert result.reason == "no_authority_context"


def test_h2_per_action_check_observes_current_authority():
    gie = GIE(); epoch = gie.grant(0); action = torch.tensor([1.0])
...
def test_h3_revocation_invalidates_cached_epoch():
    gie = GIE(); epoch = gie.grant(0); cached_action = torch.tensor([0.5, -0.25])
    gie.revoke(0)
...
def test_reauthorization_creates_new_epoch():
    ...
    gie.revoke(0)
    epoch_2 = gie.grant(0)
    assert epoch_2 == epoch_1 + 2
    old_result = gie.check(0, torch.tensor([1.0]), action_epoch=epoch_1)
    new_result = gie.check(0, torch.tensor([2.0]), action_epoch=epoch_2)
...
def test_repeated_revoke_is_idempotent():
    gie = GIE(); epoch = gie.grant(0)
    gie.revoke(0); revoked_epoch = gie.current_epoch(0); gie.revoke(0)
    assert revoked_epoch == epoch + 1
    assert gie.current_epoch(0) == revoked_epoch


def test_stale_and_revoked_prefers_stale_epoch():
    gie = GIE(); epoch = gie.grant(0); gie.revoke(0)
...
def test_explicit_context_can_be_checked_without_mutating_gie_state():
    gie = GIE(); action = torch.tensor([0.0])
    result = gie.check(env_id=7, action=action, action_epoch=4,
...
def test_byte_execution_identity_is_non_authoritative_evidence():
    gie = GIE(); epoch = gie.grant(0, epoch=481); packet = bytes(range(64))
