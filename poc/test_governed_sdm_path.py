...
def test_canonical_sdm_path_preserves_exact_packet_on_allow() -> None:
    gie = GIE(); epoch = gie.grant(env_id=0, epoch=481)
    command = _command(0, epoch); path = GovernedExecutionPath(gie, env_id=0)
...
def test_canonical_sdm_path_blocks_replay_after_epoch_change() -> None:
    gie = GIE(); epoch = gie.grant(env_id=0, epoch=481)
    command = _command(0, epoch); path = GovernedExecutionPath(gie, env_id=0)
    first = path.commit(command); assert first.decision is Decision.ALLOW
    gie.revoke(env_id=0); gie.grant(env_id=0, epoch=482)
...
def test_canonical_sdm_path_enforces_safety_independently() -> None:
    gie = GIE(); epoch = gie.grant(env_id=0, epoch=481)
    command = _command(0, epoch); path = GovernedExecutionPath(gie, env_id=0)
    result = path.commit(command, halos_decision=SafetyDecision.BLOCK, halos_reason="halos_unsafe")
...
def test_canonical_sdm_path_isolated_by_environment() -> None:
    gie = GIE(); epoch_0 = gie.grant(env_id=0, epoch=481); epoch_1 = gie.grant(env_id=1, epoch=731)
    command_0 = _command(0, epoch_0); command_1 = _command(1, epoch_1, first_byte=0xB2)
    path_0 = GovernedExecutionPath(gie, env_id=0); path_1 = GovernedExecutionPath(gie, env_id=1)
    allowed_0 = path_0.commit(command_0); allowed_1 = path_1.commit(command_1)
    assert allowed_0.decision is Decision.ALLOW; assert allowed_1.decision is Decision.ALLOW
    gie.revoke(env_id=0)
    blocked_0 = path_0.commit(command_0); still_allowed_1 = path_1.commit(command_1)
...
def test_canonical_sdm_path_rejects_command_from_different_environment() -> None:
    gie = GIE(); epoch = gie.grant(env_id=0, epoch=481)
    command = _command(1, epoch); path = GovernedExecutionPath(gie, env_id=0)
    result = path.commit(command)
    assert result.decision is Decision.BLOCK
    assert result.packet is None
