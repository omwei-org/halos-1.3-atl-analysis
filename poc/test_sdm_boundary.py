...
def test_sdm_allowed_packet_reaches_receiver_byte_for_byte():
    gie = GIE(); epoch = gie.grant(0, epoch=481); packet = b"HALOS-ATL" + bytes(range(55))
    command = SDMCommand(env_id=0, packet=packet, governance_epoch=epoch)
    boundary = ATLCommitBoundary(CommitGate()); adapter = SDMCommitAdapter(boundary); receiver = MockATLReceiver()
    ...
    result = adapter.commit(command, authority, safety); receiver.transmit(adapter.transmit_payload(command, result))
...
def test_sdm_replay_after_reauthorization_is_blocked():
    gie = GIE(); epoch_481 = gie.grant(0, epoch=481); packet = bytes(range(64))
    ...
    first = adapter.commit(command, authority_481, safety_481); receiver.transmit(adapter.transmit_payload(command, first))
    assert first.decision is Decision.ALLOW
    gie.revoke(0); gie.grant(0, epoch=482)
    ...
    replay = adapter.commit(command, stale_authority, stale_safety); receiver.transmit(adapter.transmit_payload(command, replay))
    assert replay.decision is Decision.BLOCK
    assert replay.reason == "authority_STALE_EPOCH"
    assert receiver.received == [packet]


def test_sdm_packet_mutation_after_authority_binding_is_blocked():
    gie = GIE(); epoch = gie.grant(0, epoch=481); original = bytes(range(64)); mutated = bytes([255]) + original[1:]
    boundary = ATLCommitBoundary(CommitGate()); adapter = SDMCommitAdapter(boundary)
...
def test_sdm_treats_packet_as_opaque_bytes():
    gie = GIE(); epoch = gie.grant(0, epoch=481)
    ...
    authority = gie.check_bytes_evidence(evidence, packet)
    safety = HalosAdapter.bind(evidence.action_digest, SafetyDecision.ALLOW, "halos_safe")
    result = adapter.commit(command, authority, safety); payload = adapter.transmit_payload(command, result)
    assert result.decision is Decision.ALLOW
