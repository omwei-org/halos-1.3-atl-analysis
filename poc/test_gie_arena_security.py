...
def test_check_uses_internal_gie_state_not_caller_context_when_bound_for_arena():
    ...
    gie = GIE(); gie.grant(0, epoch=7)
    forged_context = AuthorityContext(epoch=999, revoked=False)
    result = gie.check(env_id=0, action=torch.ones(4), action_epoch=7)
    ...
    forged_result = gie.check(env_id=0, action=torch.ones(4), action_epoch=999, authority_context=forged_context)
...
def test_reset_does_not_grant_authority():
    ...
    gie = GIE(); gie.grant(0, epoch=3); gie.revoke(0)
    assert gie.current_epoch(0) == 4
    result = gie.check(env_id=0, action=torch.zeros(4), action_epoch=3)
    assert result.decision is Decision.BLOCK
    assert result.reason == "STALE_EPOCH"
